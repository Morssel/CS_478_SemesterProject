...
# file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            ...
            #s3.meta.client.upload_file(filename, 'zappabucketjktest', "myUploadedFile.pdf")
            print("the file should be uploaded")
# !!! Need to change the filename in S3 so it is not the same as when it was uploaded, to prevent duplicate naming
            s3.meta.client.upload_file('myUploadedFile.pdf', 'zappabucketjktest', 'myDownloadedFile.pdf')
            # flash('File successfully uploaded')
            # global process_file

            # # process_file = s3.meta.client.download_file('zappabucketjktest', filename, 'downloadedfile.pdf')
            # outputTXT = home()
            # outputTXT = "This is the string: " + outputTXT
            # print(outputTXT)
